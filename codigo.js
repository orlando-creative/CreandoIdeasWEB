    ...
    // Generar tabla para previsualización
        function generatePreviewTable(id) {
            // Selecciona todas las tablas con el mismo ID (para manejar duplicados en HTML)
            const tables = Array.from(document.querySelectorAll(`#${id}`));
            if (tables.length === 0) return '';
        
            let rowsHtml = '';
            tables.forEach(table => {
            table.querySelectorAll('tbody tr').forEach(row => {
                const cb = row.querySelector('.seleccionar');
                if (!cb || !cb.checked) return;
        
                const producto = row.cells[0].textContent.trim();
                const specSelect = row.querySelector('select');
                const especificacion = specSelect ? specSelect.options[specSelect.selectedIndex].text : row.cells[1].textContent.trim();
                const precio = parseFloat(row.querySelector('.precio').value || 0).toFixed(2);
        
                rowsHtml += `<tr><td>${producto}</td><td>${especificacion}</td><td>${precio} Bs.</td></tr>`;
            });
            });
            ...
            <div class="table-responsive">
                <table>
                <thead><tr><th>Producto</th><th>Especificaciones</th><th>Precio</th></tr></thead>
